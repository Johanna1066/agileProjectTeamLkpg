...
direction(bool) keeps track if the engine is currently going backwards or forward.
currentVelocity(int) helps to make sure that the velocity is only set if the
velocity has actually changed since the last call.

The class has three methods; setVelocity - takes two arguments, inVelocity(int) that is rescaled
                                           using  the arduino map-funtion to the desired new velocity
                                           for the engine. The other is obsticle(bool) that should be true of there is
                                           an obsticle in the way and the engine shoul hinder
                                           movement forwards.
                             directionReverse - does the electronic switch between forward and
                                           backwards movement.
